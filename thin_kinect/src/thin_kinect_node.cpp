...
void* freenect_thread(void* v) {
  ...
  if (params->video_mode.video_format != FREENECT_VIDEO_DUMMY) {
    ...
    float hfov=M_PI/180.0*62;
    float vfov=M_PI/180.0*48.6;
  ...
  while (params->run && ! retval) {
    ...
    if (!(count%num_frames_stat)){
      ...
      if (fps<1.0) {
	printf("Warning: running at %lf fps\n", fps);
	fflush(stdout);
      }
...
int main(int argc, char **argv){
  ...
  n.param("device_id", device_id, std::string(""));
  ...
  while (aux) {
    ...
    if (aux->camera_serial==device_id) {
      device_num = k;
      printf("*** device %02d selectd from serial %s ***\n", k, aux->camera_serial);
    }
    aux = aux->next;
    k++;
  }
  ...
  ros::Duration(1.0+device_num*1.0).sleep();
  
  ROS_INFO("Kinect device %d is starting.",device_num);
